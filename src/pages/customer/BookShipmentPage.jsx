...
const BookShipmentPage = () => {
  ...
  const effectivePricePerKilo = parseFloat(selectedTrip?.price_per_kg || 0) > 0
    ? parseFloat(selectedTrip.price_per_kg)
    : pricePerKilo;
  ...
  const handleRouteChange = (label) => {
    ...
    if (route) {
      ...
      if (form.sender_province && senderSide !== expectedSender) {
        u('sender_province', ''); u('sender_city', '');
      }
  ...
  // ── "Use my registered address" Logic ──────────────────────────────────────
  ...
  useEffect(() => {
    if (useRegisteredSender && userProfile) {
      setForm(p => ({
        ...
        sender_facebook: userProfile.facebook_name || '',
        sender_lot_block: userProfile.address_lot_block || '',
        sender_street: userProfile.address_street || '',
        sender_barangay: userProfile.address_barangay || '',
        sender_city: userProfile.address_city || '',
        sender_province: userProfile.address_province || '',
        sender_landmark: userProfile.address_landmark || '',
      }));
  ...
  useEffect(() => {
    if (useRegisteredReceiver && userProfile) {
      setForm(p => ({
        ...
        receiver_facebook: userProfile.facebook_name || '',
        receiver_lot_block: userProfile.address_lot_block || '',
        receiver_street: userProfile.address_street || '',
        receiver_barangay: userProfile.address_barangay || '',
        receiver_city: userProfile.address_city || '',
        receiver_province: userProfile.address_province || '',
        receiver_landmark: userProfile.address_landmark || '',
      }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const fullSenderAddress = buildFullAddress({
        ...
        barangay: form.sender_barangay, city: form.sender_city,
        province: form.sender_province, landmark: form.sender_landmark
      });

      const fullReceiverAddress = buildFullAddress({
        lotBlock: form.receiver_lot_block, street: form.receiver_street,
        barangay: form.receiver_barangay, city: form.receiver_city,
        province: form.receiver_province, landmark: form.receiver_landmark
      });
      ...
      const payload = {
        ...
        sender_facebook: form.sender_facebook,
        ...
        receiver_facebook: form.receiver_facebook,
      ...
      // Error is displayed via setError() below
      setError(err.message || 'An unexpected error occurred while saving the booking.');
      // Scroll to the bottom so the user sees the error near the button, or to top if they prefer.
    ...
    <div className="page-transition" style={{ padding: '40px 20px' }}>
      <div className="animate-scale-in" style={{ textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
          <button className="btn btn-outline" onClick={() => navigate('/customer/orders')} style={{ flex: 1 }}>
            <Package size={16} /> View Orders
          </button>
          <button className="btn btn-primary" onClick={() => { setSuccess(null); setStep(1); setForm(p => ({ ...p, package_description: '', package_weight: '', trip_id: '' })); setUseRegisteredReceiver(false); setUseRegisteredSender(false); }} style={{ flex: 1 }}>
  ...
  return (
    <div className="page-transition">
      ...
      {error && (
        <div className="alert-banner alert-banner-error animate-slide-up" style={{ marginBottom: 16 }}>
        ...
        <div className="card animate-fade-in"><div className="card-body">
          ...
          {preTripId && (
            <div className="alert-banner alert-banner-success" style={{ marginBottom: 16 }}>
              <CheckCircle size={16} />
              Route & trip pre-selected from home page. You may change below if needed.
            </div>
          )}
              ...
              <select className="form-select" value={form.trip_id} onChange={e => u('trip_id', e.target.value)}>
                ...
                {filteredTrips.map(t => (
                  <option key={t.id} value={t.id}>
                    {t.trip_number} - {new Date(t.departure_date).toLocaleDateString()} - PHP {parseFloat(t.price_per_kg || pricePerKilo).toFixed(2)}/kg
                  </option>
                ))}
              </select>
            </div>
